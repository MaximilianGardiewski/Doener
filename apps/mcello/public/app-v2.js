...
function renderCart() {
  ...
  $("#requestOtp").disabled = state.cart.length === 0 || !state.backendReady;
}
...
async function loadSlots() {
  ...
  if (!state.backendReady) {
    state.slots = [];
    select.innerHTML = '<option value="">Lokales Backend erforderlich</option>';
    return;
  }
...
async function requestOtp() {
  ...
  if ($("#pickupMode").value === "later" && !$("#pickupSlot")?.value) {
  ...
  try {
    ...
    button.disabled = state.cart.length === 0 || !state.backendReady;
  }
}
...
async function submitOrder() {
  ...
  const requestedPickupAt = pickupMode === "later" ? $("#pickupSlot")?.value || null : null;
  if (pickupMode === "later" && !requestedPickupAt) {
  ...
  try {
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok) {
      if (data.error === "SLOT_FULL" || /slot/i.test(data.message || "")) await loadSlots();
