...
| D062 | Scope Rule | Every confirmed interview/design decision in this ledger is binding according to its explicit V1/prepared/later status, and CI must enforce ledger coverage/status discipline. | `IMPLEMENT_V1` |
...
| D066 | Mcello Originals | Existing Mcello dishes should open from their standard recipe with clear `Genau so` and `Anpassen` paths. `Anpassen` starts from the real standard configuration rather than forcing an empty builder canvas. | `IMPLEMENT_V1` |
| D067 | Experience Modes | Public pages prioritize cinematic venue/brand storytelling, while the ordering store progressively becomes denser, faster and app-like. Both modes share the same Mcello brand system without forcing identical layouts/components. | `IMPLEMENT_V1` |
| D068 | Visual Content Integrity | Until owner-confirmed real product/ingredient assets exist, builder visuals must be explicitly stylized/placeholder-derived and must not imply that generated food imagery is documentary Mcello reality. Final real food/venue media requires first-party provenance and rights confirmation. | `IMPLEMENT_V1` |
| D069 | Visual Acceptance Gates | Design acceptance requires real Mobile/Desktop screenshots and explicit visual gates for Art Direction, Brand, Homepage, Store, Builder, Cart/Checkout/Status, KDS/Admin and final real assets. Technical green tests alone do not mark visual design accepted. | `IMPLEMENT_V1` |
| D070 | Design Tool Boundary | Adobe, Figma, Canva, Lovable and similar tools are optional design/asset clients. They may accelerate exploration, but relevant decisions/tokens/assets must return to Git or the governed Media/CMS pipeline, and none may become a mandatory Mcello runtime/deployment dependency. | `IMPLEMENT_V1` |
...
| D075 | Atomic Ingredient Visual Language | Governed atomic configurator ingredients and the assembled-Döner presentation use one coherent warm photorealistic food-imagery language with deterministic repeated instances. This explicitly supersedes only D071's cartoon rendering treatment for those atomic/assembled layers; D071's coherent, adult, warm and clearly illustrative boundary remains. Generated assets stay provisional and non-documentary under D068 until first-party confirmation and rights approval. | `IMPLEMENT_V1` |
| D076 | Stacked Layer Builder Stage | The assembled Döner stage composes exactly one governed layer master per ingredient role, stacked on a shared Z axis under a single generation contract: isometric 45 degrees, direct warm top light, solid black generation background and a transparent normalized master. Flatbread is two masters, base and lid, so the filling sits between them. This supersedes only the deterministic repeated-instance clause of D075; the warm photorealistic language of D075 and the provisional, non-documentary boundary of D068 remain binding. Yufka keeps its vector vessel and requests no ingredient master. | `IMPLEMENT_V1` |
