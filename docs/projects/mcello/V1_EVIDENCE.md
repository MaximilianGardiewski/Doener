...
|---|---|---|---|
| D002 | `VERIFIED` | `apps/mcello/public/app.js`, `apps/mcello/server.mjs`, `packages/ordering/src/checkout.ts`, `tests/independent-ordering.test.mjs`, `tests/supabase-local.integration.mjs` | Der Browser sendet an Mcellos eigenes `/api/checkout`; Server/Ordering-Core validieren OTP, Katalog, Shopzustand, Slots und Preise und persistieren über die eigene Supabase-Grenze. Kein Lieferando/Wolt/Uber-Eats-artiger Marketplace ist für den Kernflow notwendig. |
...
| D044 | `VERIFIED` | `supabase/migrations/20260815032000_admin_shop_override.sql`, `supabase/migrations/20260815030100_rush_mode_functions.sql`, `apps/mcello/public/schedule.html`, `tests/admin-shop-override.integration.mjs`, `tests/ordering-schedule.integration.mjs` | Shopzustand leitet sich aus dem strukturellen Zeitplan ab. Admin kann `force_open`, `force_closed`, `pause`, `rush`, `today_closed` oder `auto` mit Operator-Hinweis setzen; Staff darf `force_open` weiterhin nicht verwenden. `force_open` übersteuert einen geschlossenen Wochenplan, Rush bleibt an Öffnungsplan/Cutoff gebunden und Pause/Closed/Today-Closed blockieren Submit. |
| D052 | `VERIFIED` | `apps/mcello/public/schedule.html`, `supabase/migrations/20260815012000_ordering_schedule_backoffice.sql`, `supabase/migrations/20260815030100_rush_mode_functions.sql`, `tests/ordering-schedule.integration.mjs` | Admin kann den Online-Bestell-Cutoff vor Schließung konfigurieren; die DB-Shop-State-/Submit-Grenze erzwingt ihn gegen den realen Öffnungsplan. Der Integrationstest belegt den konfigurierbaren 30-Minuten-Fixture und die Submit-Sperre am Cutoff. |
...
| D064 (production spend/runtime guard) | `VERIFIED` | `infra/selfhost/container-entrypoint.mjs`, `infra/selfhost/preflight.sh`, `tests/messaging-spend-boundary.test.mjs`, `.github/workflows/selfhost-release.yml` | Der echte Production-Container blockiert `/api/dev/**` vor dem internen Development-Server und liefert niemals `devCode`. Checkout bleibt mit `PRODUCTION_MESSAGING_NOT_CONFIGURED` fail-closed, solange kein freigegebener realer Transport implementiert ist. Provider-Variablen werden vom Production-Preflight ohne exakt `ALLOW_PAID_MESSAGING=YES` abgewiesen. D003/D016 bleiben bis zur realen WhatsApp/SMS-Zustellung separat offen. |
...
| D059 | `VERIFIED` | `apps/mcello/public/public-copy.js`, `apps/mcello/public/public-content.js`, `apps/mcello/public/homepage-composition.js`, `tests/public-copy-tone.test.mjs`, `tests/public-copy-tone.browser.mjs`, `.github/workflows/ci.yml` | Kundenorientierte Copy ist ruhig/premium und warm/persönlich mit einzelnen lockeren Fast-Casual-Momenten. Interne Begriffe wie Showcase/KDS/Media-Layer bleiben aus Marketingflächen heraus, während Prototype-/Operator-Hinweise wahrheitsgetreu sichtbar bleiben. Keine Namen, Historie, Superlative oder Kontaktdaten werden erfunden; Chromium beweist den gerenderten Desktop-/Mobile-Ton ohne horizontalen Overflow. |
...
| D001 | `VERIFIED` | `apps/mcello/public/styles.css`, `apps/mcello/public/manifest.webmanifest`, `tests/brand-design-system.test.mjs`, `tests/brand-design-system.browser.mjs`, `.github/workflows/ci.yml` | Semantische Anthrazit-, warmes Off-White-, Amber/Gold- und selektive Heritage-Green-Tokens steuern Public-Surfaces, Typografie, Radien, Schatten und CTAs. Statische Guards beweisen Token-Vertrag und lesbare Kontraste; Chromium beweist die tatsächlich gerenderten Tokens auf Desktop/Mobile ohne horizontalen Overflow. Self-host-Container und kompletter Supabase-Regressionsstack bleiben auf demselben Runtime-Head grün. D029 Logo/Recognition bleibt separat offen. |
| D062 | `VERIFIED` | `docs/projects/mcello/DECISIONS.md`, `docs/projects/mcello/ACCEPTANCE.md`, `docs/projects/mcello/V1_EVIDENCE.md`, `tests/decision-ledger-coverage.test.mjs` | CI parst alle D001–D064-Entscheidungen, erzwingt eindeutige/sequenzielle IDs, explizite V1-Coverage, doppelte Prepared-Abbildung und verhindert, dass `LATER_OPTION` still als V1 fertig markiert wird. Umgekehrt dürfen Acceptance/Evidence keine unbekannten Decision-IDs einführen. |
...
|---|---|---|---|
| D003 | `PARTIAL` | Provider-neutraler OTP-Contract; Start fordert `whatsapp` primär und `sms` als Fallback an; lokaler OTP-Provider ist getestet. | Echte freigegebene Production-Transporte für WhatsApp + SMS sind noch nicht aktiviert. D064 verbietet stille Providerkosten und Production fällt bis dahin geschlossen aus. |
| D037 | `PARTIAL` | Closed/Pause/Cutoff lässt Browsen/Konfigurieren/Warenkorb zu und blockiert Submit; Öffnungsplan, Overrides und Cutoff sind DB-getestet. D044 und D052 sind separat VERIFIED. | D037 verlangt zusätzlich sichtbare Telefon-/WhatsApp-Fallback-Kontakte; first-party bestätigte Kontaktdaten sind noch nicht verdrahtet. |
...
- D029 — finales Logo/Recognition-Asset mit Premium-Anwendung; der technische D001-Farb-/Surface-Vertrag ist unabhängig verifiziert
