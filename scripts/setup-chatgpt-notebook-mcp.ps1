...
$Script:Failures = @()
# Tracked separately because only some failures actually prevent the local
# bridge from starting. tunnel-client and the tunnel config are needed to reach
# ChatGPT, not to run the bridge, so they must not cascade into "bridge skipped
# -> health unreachable -> MCP checks failed" and report three problems where
# there is one.
$Script:BridgeBlockers = @()
...
function Write-Fail { param([string]$Message, [switch]$BlocksBridge)
    Write-Host "    FAIL $Message" -ForegroundColor Red
    $Script:Failures += $Message
    if ($BlocksBridge) { $Script:BridgeBlockers += $Message } }
...
if (Test-Command 'node') {
    ...
    else { Write-Fail "node $NodeVersion is too old; this repo requires >= 22" -BlocksBridge }
}
else { Write-Fail 'node is not installed (https://nodejs.org)' -BlocksBridge }
if (Test-Command 'npm') { Write-Ok "npm $(& npm --version)" } else { Write-Fail 'npm is not on PATH' -BlocksBridge }
...
if (Test-Path (Join-Path $RepoRoot 'node_modules')) { Write-Ok 'node_modules present' }
    ...
    if ($LASTEXITCODE -ne 0) { Write-Fail 'dependency install failed' -BlocksBridge } else { Write-Ok 'dependencies installed' }
}
...
if ($HaveNlm -and $HaveMcp) { Write-Ok 'nlm and notebooklm-mcp are on PATH' }
    ...
    if ($LASTEXITCODE -ne 0) { Write-Fail "setup-gemini-notebook-bridge.ps1 failed (exit $LASTEXITCODE)" -BlocksBridge }
    elseif ((Test-Command 'nlm') -and (Test-Command 'notebooklm-mcp')) { Write-Ok 'installed' }
    else { Write-Fail 'still not on PATH; open a new shell and rerun' -BlocksBridge }
}
...
if (Test-Command 'nlm') {
    ...
    else { Write-Fail "not authenticated - run 'nlm login' in this shell, then rerun" -BlocksBridge }
}
else { Write-Fail 'skipped: nlm missing' -BlocksBridge }
...
if ($LASTEXITCODE -eq 0) { Write-Ok 'allowlist guards pass' } else { Write-Fail 'allowlist guards FAILED' -BlocksBridge }

# ----------------------------------------------------- 9. start bridge ----
Write-Step '9/12  Local bridge'
$BridgeRunning = $false
if ($SkipBridgeStart) { Write-Skip 'skipped by -SkipBridgeStart' }
elseif ($Script:BridgeBlockers.Count -gt 0) {
    Write-Skip "skipped: $($Script:BridgeBlockers.Count) prerequisite(s) failed"
}
else {
    & pwsh -NoProfile -ExecutionPolicy Bypass `
        -File (Join-Path $PSScriptRoot 'start-gemini-notebook-chatgpt-mcp.ps1') `
        -Mode $Mode -Port $Port -Background
    if ($LASTEXITCODE -eq 0) { Write-Ok 'bridge started in background'; $BridgeRunning = $true }
    else { Write-Fail 'bridge failed to start' }
}

# ------------------------------------------------------ 10. healthcheck ---
Write-Step '10/12  Health endpoint'
if (-not $BridgeRunning) { Write-Skip 'skipped: the bridge was not started' }
else {
    try {
        $Response = Invoke-WebRequest -Uri $HealthUrl -TimeoutSec 5 -UseBasicParsing
        if ($Response.StatusCode -eq 200) { Write-Ok "$HealthUrl responded 200" }
        else { Write-Fail "$HealthUrl returned $($Response.StatusCode)" }
    }
