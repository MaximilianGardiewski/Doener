...
test("D062 ledger contains exactly the sequential D001-D064 decision set", () => {
  assert.equal(decisions.length, 64, `expected 64 decision rows, got ${decisions.length}`);
  assert.equal(new Set(decisions.map(({ id }) => id)).size, 64, "decision IDs must be unique");
  assert.deepEqual(
    decisions.map(({ id }) => id),
    Array.from({ length: 64 }, (_, index) => `D${String(index + 1).padStart(3, "0")}`),
  );
});
