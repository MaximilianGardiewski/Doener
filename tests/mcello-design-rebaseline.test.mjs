...
const [decisions, acceptance, designAcceptance, masterplan, journeys, roadmap, presentationV4] = await Promise.all([
  ...
  read("docs/projects/mcello/PRESENTATION_UX_V4.md"),
]);

const combined = [acceptance, designAcceptance, masterplan, journeys, roadmap, presentationV4].join("\n");

test("design rebaseline keeps D065-D073 explicit and tracked", () => {
  for (let number = 65; number <= 73; number += 1) {
...
test("interactive builder remains a visual layer over existing domain authority", () => {
  ...
  assert.match(presentationV4, /never mutates them/);
});
...
test("visual integrity forbids fake documentary Mcello reality", () => {
  ...
  assert.match(decisions, /warm editorial cartoon-food language/);
  assert.match(presentationV4, /never described as a real Mcello product photo/);
});
...
test("operations IA and handbook are now explicit V1 presentation contracts", () => {
  assert.match(decisions, /Operations Information Architecture/);
  assert.match(decisions, /Integrated Handbook/);
  assert.match(presentationV4, /Staff remains operational-only/);
  assert.match(presentationV4, /docs\/projects\/mcello\/handbook/);
});
...
test("design tools stay optional and outputs return to governed sources", () => {
  ...
  assert.match(presentationV4, /runtime must never depend on the design connector/);
});
